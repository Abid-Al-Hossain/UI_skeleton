# UI Foundry Skeleton Studio

Standalone visual Skeleton generator built with Next.js, React, and TypeScript.

Standalone Skeleton component studio for building native, preview-honest, React-exportable UI patterns that match the UI Foundry product line.
...
- designers building Skeleton UI quickly
- developers who need editable React starting points
- marketplace buyers who want a self-contained Skeleton generator
...
The editor is focused on controls that are native to Skeleton and useful in real product work:
- line count, avatar/media block, card/table/list layout, shimmer/pulse/static mode, and reduced-motion fallback
- radius, spacing, color, opacity, duration, gap, and container treatment
- article, profile, table, card, dashboard, and media skeleton archetypes
- aria-hidden or loading-context behavior guidance
- React export for placeholder layout

The goal is maximum useful depth without adding unrelated controls that would make the component stop reading as Skeleton.
...
This repo is intended to publish as Abid-Al-Hossain/UI_skeleton.
